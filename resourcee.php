											   ...
											   <div id="error1" style=" text-align:center; color:#FF0000"></div>
												...
												<h3 style="text-align:center;color:#FFFFFF;">Resources</h3><br>
													...
													<?php
														...
														{
													?>
													<a href="admin/repfaculty.php" style="cursor:pointer;" class="btn btn-primary">Cancel</a>
													<?php } ?>
                                                    </div>
                                                </form>
                                                </div>
...
<?php 
$sel1=executework("select * from skillistic_resource where status='1' order by id desc");
$count=@mysqli_num_rows($sel1);
	...
	$sel=executework("select * from skillistic_resource where status='1' order by id desc LIMIT $pagenow1, $max_recs_per_page");
	if($count>0)
	{
?>
                                                <table class="table border-table" bordercolor="#FFFFFF" style="color:white;" align="center">
                                        <thead>
                                            <tr>
                                                <th>#</th>
												<th>Title</th>
...
<?php
	  for($im=1;$im<=$pages;$im++)
	  {
		  if($page12 == $im)
			...
			$ac='';
?>
               <li <?php echo $ac; ?>  class="page-number <?php if(!empty($_GET['page_index'])&&($_GET['page_index']==$ac)){echo "active"; }?>"> <a href="admin/resource.php?page_index=<?php echo $im ?>"><?php echo $im; ?></a> </li>
		<?php
	  }
		?>
