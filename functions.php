...
function master_insert()
{
	if($_GET['id']!=6)
	{
		...
		for($i=1;$i<=5;$i++)
		{
			...
			{
				$sel=executework("select * from skillistic_masterdata where master_type='".$_GET['master_name']."' and catid=".$catid." and master_value='".$_POST['master_name'.$i]."'");
				$count=@mysqli_num_rows($sel);
				if($count>0)
				{
					//redirect("masterdata.php?exist=1&id=".$_GET['id']."&master_name=".$_GET['master_name']);
					$exst++;
				}
				else
				{
		...
		redirect("masterdata.php?id=".$_GET['id']."&master_name=".$_GET['master_name']."&succ=".$succ."&exst=".$exst);
	}
	else if($_GET['id']==6)
	{
...
function faculty_insert()
{
	...
	else
	{
		...
		{
			$sele=executework("select * from skillistic_faculty where faculty_id='".$_POST['faculty_id']."'");
			$cnte=@mysqli_num_rows($sele);
			if($cnte>0)
			{
				$exst=1;
			}
...
{
	$sel=executework("select * from skillistic_faculty order by faculty_name");
	$arr=array();
	while($row=@mysqli_fetch_array($sel))
	{
		$arr[]=$row;
...
function programs_insert()
{
 $qr='';
	  	...
	  	else
		{
			...
			if(!empty($_GET['eid']))
			{
				$update=executework("update skillistic_programs set category='".$cat."',prgrm_title='".$_POST['prgrm_title']."',modified_on='".date("Y-m-d H:i:s")."' where id='".$_GET['eid']."'");
				redirect("programme_mgmt.php?upd=1");
			}
			else
			{
				$insert=executework("insert into skillistic_programs(id,category,prgrm_title,status,created_on)values('','".$cat."','".$_POST['prgrm_title']."','1','".date("Y-m-d H:i:s")."')");
				//echo ("insert into skillistic_programs(id,category,prgrm_title,status,created_on)values('','".$cat."','".$_POST['prgrm_title']."','1','".date("Y-m-d H:i:s")."')");
				redirect("programme_mgmt.php?succ=1");
			}
	}
}
...
{
	$sel=executework("select * from skillistic_programs order by prgrm_title asc");
	$arr=array();
	while($row=@mysqli_fetch_array($sel))
	{
		$arr[]=$row;
...
function ppt_insert()
{
     $qr='';
	  	...
	  	else
		{
			...
			{
				$insert=executework("insert into skillistic_ppt(id,category,prgrm_title,topic,details,ppt,status,created_on)values('','".$_POST['category']."','".$_POST['prgrm_title']."','".$_POST['topic']."','".$_POST['details']."','".$file_name."','1','".date("Y-m-d H:i:s")."')");
...
function schedule_insert()
{
	...
	//$del_date=executework("delete from skillistic_schedule where faculty_id='".$fac_id."'");
	
	for($j=0;$j<$_POST['date_count'];$j++)
	{	
		for($k=0;$k<$_POST['fac_count'];$k++)
		{
			//$sdate=date("Y-m-d",strtotime($_POST['sc_date'.$i]));
			$sdate=$_POST['sc_date'.$k."h".$j];
			//echo "sdate".$sdate;
			//exit();
			if(!empty($_POST['institute'.$k."h".$j]))
			{
			//echo "hii";
			//exit();
				$sel=executework("select * from skillistic_schedule where faculty_id='".$_POST['fac'.$k."h".$j]."' and sdate='".$sdate."'");
				//echo ("select * from skillistic_schedule where faculty_id='".$_POST['fac'.$k."h".$j]."' and sdate='".$sdate."'");
				//echo ("select * from skillistic_schedule where faculty_id='".$_POST['faculty_id'.$k."h".$j]."' and sdate='".$sdate."'");
				...
				{
					//echo "hello";
					$update=executework("update skillistic_schedule set institution_id='".$_POST['institute'.$k."h".$j]."',programme_id='".$_POST['programme'.$k."h".$j]."',stime='".$_POST['time'.$k."h".$j]."',modified_on='".date("Y-m-d H:i:s")."',sdate='".$sdate."' where faculty_id='".$_POST['fac'.$k."h".$j]."' and sdate='".$sdate."'");
					//echo ("update skillistic_schedule set institution_id='".$_POST['institute'.$k."h".$j]."',programme_id='".$_POST['programme'.$k."h".$j]."',stime='".$_POST['time'.$k."h".$j]."',modified_on='".date("Y-m-d H:i:s")."',sdate='".$sdate."' where faculty_id='".$_POST['fac'.$k."h".$j]."' and sdate='".$sdate."'");
				...
				//echo "faculty".$faculty_val;
					$insert=executework("insert into skillistic_schedule(id,faculty_id,institution_id,programme_id,stime,city,created_on,sdate,status,sc_status)values('','".$_POST['fac'.$k."h".$j]."','".$_POST['institute'.$k."h".$j]."','".$_POST['programme'.$k."h".$j]."','".$_POST['time'.$k."h".$j]."','".$_POST['instcity'.$k."h".$j]."','".date("Y-m-d H:i:s")."','".$sdate."','Scheduled','1')");
				//	echo ("insert into skillistic_schedule(id,faculty_id,institution_id,programme_id,stime,city,created_on,sdate,status,sc_status)values('','".$_POST['fac'.$k."h".$j]."','".$_POST['institute'.$k."h".$j]."','".$_POST['programme'.$k."h".$j]."','".$_POST['time'.$k."h".$j]."','".$_POST['instcity'.$k."h".$j]."','".date("Y-m-d H:i:s")."','".$sdate."','Scheduled','1')");
					//exit();
					//echo ("insert into skillistic_schedule(id,faculty_id,institution_id,programme_id,stime,created_on,sdate,status,sc_status)values('','".$_POST['fac'.$k.$j]."','".$_POST['institute'.$k.$j]."','".$_POST['programme'.$k.$j]."','".$_POST['time'.$k.$j]."','".date("Y-m-d H:i:s")."','".$sdate."','Scheduled','1')");
					//exit();
...
function support_insert()
{
 $qr='';
	...
	{
		$insert=executework("insert into skillistic_support(id,title,filetype,filename,created_on,status)values('','".$_POST['title']."','".$_POST['filetype']."','".$file_name."','".date("Y-m-d H:i:s")."','1')");
...
function resource_insert()
{
 $qr='';
	...
	{
		$insert=executework("insert into skillistic_resource(id,title,filename,created_on,status)values('','".$_POST['title']."','".$file_name."','".date("Y-m-d H:i:s")."','1')");
		// echo "insert into skillistic_support(id,title,filetype,filename,created_on)values('','".$_POST['title']."','".$_POST['filetype']."','".$file_name."','".date("Y-m-d H:i:s")."')";
		// exit();
