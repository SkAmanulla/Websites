...
function bulk_del(st)
{
	if(confirm("Do You Want to Delete?"))
	{
		document.form1.bulk.value=st;
		document.form1.submit();
		return true;
	}
	else
...
function convert_date(cdate,format1,format2)
{
	cdate=cdate.replace('-','/');
	var dat=cdate.split('/');
	var d,m,y;
	...
	else if(format1=='mm/dd/yyyy')
	{
		d=dat[1];
		m=dat[0];
		y=dat[2];
	}
	else if(format1=='yyyy/mm/dd')
	{
		d=dat[2];
		m=dat[1];
		y=dat[0];
...
function validate_hrs_past(cdate,tim)
{
	cdate=cdate.replace('-','/');
	var dat=cdate.split('/');
	var now=new Date();
	var ndate=now.getDate()+"/"+(now.getMonth()+1)+"/"+now.getFullYear();
	var hrs=now.getHours()+1;
	if(cdate==ndate && hrs>=tim)
	return false;
	else
	return true;
}