                                    ...
                                    <div class="row">
                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" align="center">
                                            <div class="basic-login-inner">
                                                <h3 style="text-align:center;color:#FFFFFF">Expenses Report</h3><br>
...
<?php
	$sel=executework("select * from skillistic_faculty_schedule where id!='' and sdate between '$from_date' and '$to_date' $faculty $inst order by sdate,faculty_id desc");
	...
	$pagenow1 = ($max_recs_per_page*($page12-1));
									//echo ("select * from skillistic_faculty_schedule where id!='' and sdate between '$from_date' and '$to_date' $faculty $inst $status $program order by sdate,faculty_id desc");
	$sel1=executework("select * from skillistic_faculty_schedule where id!='' and sdate between '$from_date' and '$to_date' $faculty $inst order by sdate,faculty_id desc LIMIT $pagenow1, $max_recs_per_page");
									//echo "count".$count;
									//exit();
								?>
							<?php 
...
if($pages > 1)
{
		...
		{
			$pr='';
			$prv='repexpenses.php?page_index='.($page12-1).'&from_date='. $from_date.'&to_date='.$to_date.'&faculty='.$faculty_id.'&institute='.$inst_id;
		}
		if($page12 == $pages)
		{
			$nx="class=disabled";
		...
		{
			$nx='';
			$nxt='repexpenses.php?page_index='.($page12+1).'&from_date='. $from_date.'&to_date='.$to_date.'&faculty='.$faculty_id.'&institute='.$inst_id;
		}
?>
...
<?php
	  for($im=1;$im<=$pages;$im++)
	  {
		  if($page12 == $im)
			...
			$ac='';
?>
               <li <?php echo $ac; ?>  class="page-number <?php if(!empty($_GET['page_index'])&&($_GET['page_index']==$ac)){echo "active"; }?>"> <a href="repexpenses.php?page_index=<?php echo $im ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&faculty=<?php echo $faculty_id; ?>&institute=<?php echo $inst_id; ?>"><?php echo $im; ?></a> </li>
		<?php
	  }
		?>
